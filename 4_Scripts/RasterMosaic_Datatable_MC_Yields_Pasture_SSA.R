...
pasture_2010_vector <- cbind(c(getValues(pasture_2010)))/100
...
pasture_rasters_long <- pasture_rasters_long[grep('mean', pasture_rasters_long)]
...
for(y in 1:length(yrs)) {
  ...
  # Updating values for missing countries
  tmp.pasture.vector[tmp.pasture.vector == -1] <- pasture_2010_vector[tmp.pasture.vector == -1]
  
  # And also calculating delta crop extent
  delta.ag.vector <- tmp.pasture.vector - pasture_2010_vector
