...
crop_2010_vector <- cbind(c(getValues(crop_2010)))/100
...
crop_rasters_long <- crop_rasters_long[grep('mean', crop_rasters_long)]
...
for(y in 1:length(yrs)) {
  ...
  tmp.crop.vector[tmp.crop.vector == -1] <- crop_2010_vector[tmp.crop.vector == -1]
  
  # And also calculating delta crop extent
  delta.ag.vector <- tmp.crop.vector - crop_2010_vector
