...
predictor_vars_crop <- c("dist_50k",
                         "ag_suitability",
                         "PA_binary",
...
for (k in c(10,12)) {
  ...
  dat_orig$crop_extent_change_2007.2002 <- dat_orig$Prop.Crop.2007 - dat_orig$Prop.Crop.2002
  dat_orig$crop_extent_change_2012.2007 <- dat_orig$Prop.Crop.2012 - dat_orig$Prop.Crop.2007
  # creating column for has_changed_crop between 2007 and 2012
  dat_orig$has_changed_crop_2012.2007 <- NA
  dat_orig$has_changed_crop_2012.2007[dat_orig$crop_extent_change_2012.2007 >= cutoff] <- "Increase"
  dat_orig$has_changed_crop_2012.2007[dat_orig$crop_extent_change_2012.2007 <= -cutoff] <- "Decrease"
  dat_orig$has_changed_crop_2012.2007[abs(dat_orig$crop_extent_change_2012.2007) < cutoff] <- "aNoChange"
  ...
  dat_orig <- dplyr::select(dat_orig,
                            ...
                            prop_adj_to_predict_from_crop = Prop.AdjacentCrop.2007,
                            prop_to_predict_from_crop = Prop.Crop.2007,
                            prop_to_predict_from_cell_delta_crop = crop_extent_change_2007.2002,
                            crop_extent_change_2012.2007 = crop_extent_change_2012.2007,
                            prop_to_predict_from_pasture = Prop.Pasture.2007,
                            prop_crop_cell_2002 = Prop.Crop.2002,
                            prop_crop_cell_2012 = Prop.Crop.2012,
                            prop_change_cell_crop_2007.2012 = crop_extent_change_2012.2007,
                            has_changed_crop = has_changed_crop_2012.2007,
                            GDD_10c = GDD_10c)
  # Data prep: Getting model data
  # Limiting to specific variables and complete cases
  dat_mod <- dat_orig[,c(predictor_vars_crop, "has_changed_crop", "prop_change_cell_crop_2007.2012","ISO_numeric")]
  ...
  dat_region_increase <- dat_mod[dat_mod$prop_change_cell_crop >= cutoff,]
  dat_region_decrease <- dat_mod[dat_mod$prop_change_cell_crop <= -cutoff,]
  
  # removing data
  rm(dat_mod)
  
  # Dropping rows with values <0 for increaes and > 0 for decrease
  dat_region_increase <- dat_region_increase[dat_region_increase$prop_change_cell_crop_2007.2012 >= 0,]
  dat_region_decrease <- dat_region_decrease[dat_region_decrease$prop_change_cell_crop_2007.2012 <= 0,]
  ...
  objModel_increase <- glm(prop_change_cell_crop_2007.2012 ~ 
                            dist_50k_log +
                            dist_50k_log_sq +
                            ag_suitability +
                            PA_binary +
  ...
  dat_region_decrease$prop_change_cell_crop_2007.2012 <- abs(dat_region_decrease$prop_change_cell_crop_2007.2012)
  objModel_decrease <- glm(prop_change_cell_crop_2007.2012 ~ 
                             dist_50k_log +
                             dist_50k_log_sq +
                             ag_suitability +
                             PA_binary +
  ...
  save(objModel_decrease, objModel_increase, objModel,
       file = paste("/Users/maclark/Desktop/Mike's Files/GBM Model Outputs/8_March/Gamma_Log/",
                    region_list[k],
                    "_Gamma_Link_Log_Multinom_Crop_Model.rda",
                    sep = ""))
  
  rm(objModel_decrease, objModel_increase)
  ...
  #                   sep = ""))
  save(coef.increase, coef.decrease, coef.multinom,
       file = paste("/Users/maclark/Desktop/Mike's Files/GBM Model Outputs/8_March/Gamma_Log/",
                    region_list[k],
                    "_Gamma_Link_Log_Multinom_Crop_Coefs.rda",
                    sep = ""))
